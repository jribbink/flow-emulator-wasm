...
// The server wraps an emulated blockchain instance with the Access API gRPC handlers.
 ...
 func NewEmulatorServer(logger *logrus.Logger, conf *Config) *EmulatorServer {
	 ...
	 grpcServer := NewGRPCServer(logger, be)
	 restServer, err := NewRestServer(be)
	 if err != nil {
	 ...
	 server.admin = NewAdminServer(server, be, &store, grpcServer, livenessTicker, conf.HTTPHeaders)
 ...
 func (s *EmulatorServer) Start() {
	 ...
	 s.logger.
		 WithField("port", s.config.HttpWasmKey).
		 Infof("🌱  Starting HTTP WASM API with key %d", s.config.HttpWasmKey)
	 s.group.Add(s.rest)
